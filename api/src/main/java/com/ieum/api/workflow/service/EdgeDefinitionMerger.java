 ...
 *       정의(IEUM-BE-65 이전 저장분)의 엣지에는 id가 없으므로 이 경로가 계속 필요하다. 요청이 값을
 *       보냈어도 큐는 하나 소비한다 — 안 그러면 같은 키의 다음 요청 엣지가 이미 짝지어진 이전
 *       엣지를 다시 집는다.</li>
 * </ol>
 * 요청 쪽 중복 id는 하나만 id로 짝지어지고 나머지는 FIFO로 내려간다. 중복을 거절하지는 않는다
 * (노드 id와 같은 선이다) — 그래서 형제 두 분기의 {@code conditionType}이 서로 뒤바뀔 수 있으나,
 * FIFO가 남은 짝을 집어 주므로 분기가 통째로 사라지지는 않는다.
