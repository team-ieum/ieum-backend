...
3. **커버되지 않은 새 패턴** — 변경된 파일을 읽고 스킬이 검사하지 않는 새로운 규칙, 설정, 패턴을 식별합니다.
...
2. **사용자에게 스킬 이름 확인** — 스킬이 커버할 패턴/도메인을 제시하고, 사용자에게 이름 확인 요청

   **이름 규칙:**
   - 이름은 반드시 `verify-`로 시작 (예: `verify-auth`, `verify-api`, `verify-caching`)
   - kebab-case 사용 (예: `verify-error-handling`)

3. **생성** — `.claude/skills/verify-<name>/SKILL.md`를 템플릿에 따라 생성

4. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 3개 파일을 업데이트:
...
1. **Lock 파일 및 생성된 파일** — `package-lock.json`, `yarn.lock`, `Cargo.lock`, 빌드 출력물은 스킬 커버리지 불필요
2. **일회성 설정 변경** — `build.gradle` 버전 범프, 린터 설정의 사소한 변경은 새 스킬 불필요
3. **문서 파일** — `README.md`, `CHANGELOG.md`, `LICENSE` 등은 코드 패턴이 아님
4. **테스트 픽스처 파일** — 테스트 픽스처 디렉토리의 파일은 프로덕션 코드 아님
5. **영향받지 않은 스킬** — UNAFFECTED 스킬은 검토 불필요
6. **CLAUDE.md 자체** — 문서 업데이트이며 코드 패턴 아님
7. **벤더/서드파티 코드** — `vendor/`, `node_modules/` 파일은 외부 규칙을 따름
8. **CI/CD 설정** — `.github/`, `Dockerfile` 등은 인프라이며 검증 스킬 불필요
